...
class player_controller(Controller):
    def __init__(self, network):
        # Store the NEAT network
        self.network = network

    def control(self, inputs, controller=None):
        """Uses NEAT's neural network to control the player. The NEAT network decides actions based on inputs."""

        # Normalize inputs using min-max scaling to [0, 1]
        min_in = np.min(inputs)
        max_in = np.max(inputs)
        if max_in - min_in != 0:
            inputs = (inputs - min_in) / (max_in - min_in)
        else:
            inputs = inputs  # Avoid division by zero if all inputs are the same

        # Activate NEAT neural network
        output = self.network.activate(inputs)

        # Decode output into actions (left, right, jump, shoot, release)
        actions = [1 if o > 0.5 else 0 for o in output]

        # Ensure the action list has exactly 5 elements
        return actions[:5]
